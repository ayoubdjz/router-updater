...
def run_update_procedure(ip, username, password, image_file, update_logs=None):
    ...
    try:
        ...
        update_logs.append(f"Erreur de connexion au routeur: {str(e)}")
    ...
    try:
        ...
        while (time.time() - start_time) < timeout:
            ...
            if chunk:
                ...
                time.sleep(1)
        if not re1_ready:
            update_logs.append("15 minutes dépassé - RE1 n'a pas restauré son état opérationnel")
    ...
    try:
        ...
        if current_version == expected_version:
            ...
            update_logs.append(f"ERREUR: La version sur RE1 ({current_version}) ne correspond pas à la version attendue ({expected_version})")
    ...
    try:
        ...
        while (time.time() - start_time) < timeout:
            ...
            if chunk:
                ...
                time.sleep(1)
        if not re0_ready:
            update_logs.append("15 minutes dépassé - RE0 n'a pas restauré son état opérationnel")
    ...
    try:
        ...
        if current_version == expected_version:
            ...
            update_logs.append(f"ERREUR: La version sur RE0 ({current_version}) ne correspond pas à la version attendue ({expected_version})")
            return {'success': False, 'logs': update_logs, 'error': 'Version RE0 incorrecte'}
    except Exception as e:
        update_logs.append(f"\n✗ Erreur lors de la vérification de version: {str(e)}")
        return {'success': False, 'logs': update_logs, 'error': str(e)}
            ...
            try:
                ...
                for i, line in enumerate(lines):
                    if "Slot 0" in line:
                        slot0_index = i
                        break
                if slot0_index is not None and slot0_index + 1 < len(lines):
                    next_line = lines[slot0_index + 1]
                    if "Current state" in next_line and "Master" in next_line:
                        update_logs.append("✓ Basculement vers RE0 réussi")
                        break
                    else:
                ...
                if attempt < 5:
                    ...
                    return {'success': False, 'logs': update_logs, 'error': 'Échec de reconnexion après basculement final'}
    except Exception as e:
